...
abstract class DocxEntry {
  ...
  void _updateData(Archive arch, List<int> data) {
    // Force copy into a mutable list to avoid 'Cannot modify an unmodifiable list'
    final mutableData = List<int>.from(data);
    if (_index < 0) {
      arch.addFile(ArchiveFile(_name, mutableData.length, mutableData));
    } else {
      arch.files[_index] = ArchiveFile(_name, mutableData.length, mutableData);
    }
  }
}
