...
class DocxTemplate {
  ...
  static Future<DocxTemplate> fromBytes(List<int> bytes) async {
    ...
    final rawArch = ZipDecoder().decodeBytes(bytes, verify: true);

    // Copy into a fresh mutable Archive to prevent 'unmodifiable list' crashes
    final arch = Archive();
    for (var f in rawArch.files) {
      arch.addFile(f);
    }
...
//   }
  ...
  Future<List<int>?> generate(Content c,
    ...
    return enc.encode(_manager.arch, level: DeflateLevel.bestSpeed);
  }
}
